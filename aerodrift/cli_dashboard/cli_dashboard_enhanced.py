...
"""
    ...
    def display_topology_tree(self, topology_engine, title: str = "Cloud Topology"):
        """
        Display the cloud topology as an interactive text tree with emojis.
    ...
    def _add_connected_resources(self, topology_engine, branch, resource_id):
        ...
        for connected_id in connected:
            ...
            if is_exposed:
                resource_branch = branch.add(f"[bold red]⚠️ {icon} {resource.name}[/bold red]")
            else:
    ...
    def display_incident_report(self, drift_events: List[Any], topology_stats: Dict[str, Any]):
        ...
        summary_table.add_row("❤️ Health Score", f"[{health_color}]{health_score}/100[/{health_color}]")
        summary_table.add_row(f"{self.emoji_map['critical']} Critical Events", f"[bold red]{critical_count}[/bold red]")
        summary_table.add_row(f"{self.emoji_map['high']} High Severity", f"[bold orange]{high_count}[/bold orange]")
        ...
        footer = Panel(
            Text.assemble(
                ...
                (f" {self.emoji_map['success']}", "green")
            ),
            box=ROUNDED,
            padding=(0, 2),
            border_style="dim"
    ...
    def create_monitoring_layout(self, topology_engine, drift_events: List[Any]) -> Layout:
        ...
        stats_text = Text.assemble(
            ...
            (" | ", "dim"),
            ("🗄️ Exposed DBs: ", "red" if stats['exposed_databases'] > 0 else "green"),
        ...
        if drift_events:
            ...
            for event in recent_events:
                severity_emoji = self.emoji_map.get(event.severity.value, '❓')
                events_text.append(f"{severity_emoji} ", style="bold")
                events_text.append(f"{event.description}\n", style="white")
        else:
            events_text = Text.assemble(
                (f"{self.emoji_map['success']} No recent events - System is healthy!", "green")
            )
    ...
    def print_success(self, message: str):
        ...
        self.console.print(f"[green]{self.emoji_map['success']} {message}[/green]")
    
    def print_error(self, message: str):
        """Print an error message with emoji."""
        self.console.print(f"[red]{self.emoji_map['error']} {message}[/red]")
    
    def print_warning(self, message: str):
        """Print a warning message with emoji."""
        self.console.print(f"[yellow]{self.emoji_map['warning']} {message}[/yellow]")
    
    def print_info(self, message: str):
        """Print an info message with emoji."""
        self.console.print(f"[blue]{self.emoji_map['info']} {message}[/blue]")
